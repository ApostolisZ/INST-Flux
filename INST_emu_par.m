function INST_emu_par(analysis_name, nloop, init_fracs_zero, seed)
...
for iloop = 1:nloop
    ...
    x0(1:nrxn,iloop) = v0;
    x0(nrxn + (1:nmetab),iloop) = conc0;
    x0(nrxn + nmetab + (1:nnz_iso),iloop) = fracs0;
    x0(nrxn + nmetab + nnz_iso + (1:nlmetab),iloop) = ones(nlmetab, 1);
    
    if any(x0(:,iloop) < lb) || any(x0(:,iloop) > ub) || any(abs(Aeq * x0(:,iloop) - beq) > TOL)
        fprintf('x0 does not satisfy constraints.\n');
        continue;
    end
    
    fprintf('RMS standardized error of initial point: %g\n', ...
        sqrt(INST_sqerror(x0(:,iloop), emumodel, ldata, nz_isos) ...
        / sum(cellfun(@numel, ldata.mids))));
end
parfor iloop = 1:nloop
    tic;       
    options = optimset('Display', 'iter', 'GradObj', 'on', 'Algorithm', 'interior-point');
    [xs{iloop}, fvals{iloop}, exitflags{iloop}, outputs{iloop}] = fmincon(@(x) INST_sqerror(x, emumodel, ldata, nz_isos), ...
        x0(:,iloop), A, b, Aeq, beq, lb, ub, [], options);
    toc;

    fprintf('RMS standardized error of final point: %g\n', ...
        sqrt(fvals{iloop} / sum(cellfun(@numel, ldata.mids))));
end

for iloop = 1:nloop
    x = xs{iloop};
    fval = fvals{iloop};
    exitflag = exitflags{iloop};
    output = outputs{iloop};
    ...
    save(['results/' analysis_name '/INST_emu_' identifier '_' num2str(iloop) ], ...
